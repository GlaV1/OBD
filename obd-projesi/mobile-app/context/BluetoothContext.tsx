...
// DEV_MODE=true  → simüle veri, Bluetooth gerekmez
// DEV_MODE=false → HC-06 gerçek bağlantı (OBD gelince)
// ============================================================
...
export const DEV_MODE = true; // OBD gelince false yap
...
function useSimulatedData(active: boolean) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setData(prev => ({
        rpm,
        speed:      Math.max(0, Math.min(220, Math.round(rpm / 35))),
...
export function BluetoothProvider({ children }: { children: ReactNode }) {
  const [status, setStatus] = useState<ConnectionStatus>(
    DEV_MODE ? 'connected' : 'disconnected'
  );
          ...
          try {
            ...
            if (typeof p.rpm === 'number') {
              setRealData({
                rpm:        p.rpm        ?? 0,
                speed:      p.speed      ?? 0,
                engineTemp: p.engineTemp ?? 0,
                battery:    p.battery    ?? 12.4,
                throttle:   p.throttle   ?? 0,
                fuelLevel:  p.fuelLevel  ?? 75,
                dtcs:       p.dtcs       ?? [],
              });
              setLastError(null);
            }
  ...
  // Arka plandan öne gelince yeniden bağlan
  useEffect(() => {
    if (DEV_MODE) return;
    const sub = AppState.addEventListener('change', (s: AppStateStatus) => {
      if (s === 'active' && status === 'error' && connectedDevice) connect(connectedDevice);
    });
    return () => sub.remove();
...
export function getBTStatusText(status: ConnectionStatus): string {
  switch (status) {
    case 'connected':    return DEV_MODE ? '🛠 Geliştirme Modu' : 'OBD Cihazı Bağlı';
    case 'connecting':   return 'OBD Cihazı Bağlanıyor...';
    case 'scanning':     return 'Cihaz Aranıyor...';
    case 'error':        return 'OBD Bağlantı Hatası';
    case 'disconnected': return 'OBD Cihazı Bağlı Değil';
  }
}
